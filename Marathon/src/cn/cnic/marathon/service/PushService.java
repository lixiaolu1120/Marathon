...
public class PushService extends Service {
	...
	// MQTT broker.
	public static String MQTT_CLIENT_ID = "mara";
	...
	public static void actionStart(Context ctx) {
		MQTT_HOST = ctx.getResources().getString(R.string.mqtt_host);
		Log.i(TAG, "start");
	...
	private synchronized void connect() {
		...
		if (null == uid)
			return;
		String topicName = Utils.MD5(user.getUid());
		try {
			log("MQTT_HOST is " + MQTT_HOST);
			mConnection = new MQTTConnection(MQTT_HOST, topicName);
			setStarted(true);
	 ...
	 * @param text
	 */
	private void handlePush(String text) {
		Log.d(TAG, text);
		Pattern p = Pattern.compile(Regexps.PUSH_TYPE);
	...
	private class MQTTConnection implements MqttSimpleCallback {
		...
		private void subscribeToTopic(String topicName) throws MqttException {
			...
			if ((mqttClient == null) || (mqttClient.isConnected() == false)) {
				...
				String[] topic = { "*" };
				mqttClient.subscribe(topic, MQTT_QUALITIES_OF_SERVICE);
				log("subscribe succeed, topic is " + topics[0]);
			}
		}
